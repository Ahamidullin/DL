...
def plot_losses(train_losses: List[float], val_losses: List[float]):
    ...
    """
    train_perplexities, val_perplexities = [np.exp(loss) for loss in train_losses], [np.exp(loss) for loss in val_losses]
...
def training_epoch(model: LanguageModel, optimizer: torch.optim.Optimizer, criterion: nn.Module,
    ...
    for indices, lengths in tqdm(loader, desc=tqdm_desc):
        ...
        # логиты: предсказание 1ого логита для 2 индексв => предскзание iого логита для i+1 индекса
        # надо выровнять. Убрать в токенах пад 
        indices = indices[:,1:]
        logits = logits[:, :-1] # после <eos> убираем

        # loss = criterion(logits, indices)
        loss = criterion(logits.reshape(-1, model.vocab_size), indices.reshape(-1))
        loss.backward()
        optimizer.step()
        train_loss += loss.item() * indices.shape[0] # лосс батча (сумма лосов в батче). train_loss сумма лосов всех батчей
