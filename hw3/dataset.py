...
class TextDataset(Dataset):
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, int]:
        ...
        """
        tokens = [self.bos_id] + self.indices[item][:self.max_length - 2] + [self.eos_id]
        length = len(tokens)
        tokens_padded = tokens + [self.pad_id] * (self.max_length - length)
        indices = torch.tensor(tokens_padded)
